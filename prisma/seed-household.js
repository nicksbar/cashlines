...
function getDateInMonth(day, month, year) {
  const now = new Date()
...
async function main() {
  ...
  const child1 = await prisma.person.upsert({
    where: { userId_name: { userId: user.id, name: 'Emma' } },
    update: {
      role: 'dependent',
      color: '#F59E0B',
    },
  ...
  const child2 = await prisma.person.upsert({
    where: { userId_name: { userId: user.id, name: 'Jake' } },
    update: {
      role: 'dependent',
      color: '#EC4899',
    },
  ...
  const checking = await prisma.account.upsert({
    ...
    update: { personId: sarah.id, isActive: true },
    create: {
      userId: user.id,
  ...
  const creditCard = await prisma.account.upsert({
    ...
    update: { creditLimit: 25000, personId: sarah.id, isActive: true },
    create: {
      userId: user.id,
      name: 'Chase Sapphire',
      type: 'credit_card',
  ...
  const mikeCard = await prisma.account.upsert({
    ...
    update: { creditLimit: 15000, personId: mike.id, isActive: true },
    create: {
      userId: user.id,
      name: 'Amex Blue',
      type: 'credit_card',
